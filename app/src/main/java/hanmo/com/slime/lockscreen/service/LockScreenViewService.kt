...
class LockScreenViewService : Service() {
    ...
    private val LOCK_OPEN_OFFSET_VALUE = 50f
    ...
    private fun isLockScreenAble() : Boolean? {
        return RealmHelper.instance.getLockScreenIsLock()
    }
    ...
    private fun addLockScreenView() {
        if (mWindowManager != null && mLockscreenView != null && mParams != null) {
    ...
    private fun dettachLockScreenView() : Boolean? {
        return if (isAttachedToWindow()!! && mWindowManager != null && mLockscreenView != null) {
    ...
    private fun settingLockView() {
        ...
        mLockscreenView?.let {
            ...
            mBackgroundLayout = it.findViewById(R.id.lockscreen_background_layout)
            mBackgroundInLayout = it.findViewById(R.id.lockscreen_background_in_layout)
            mBackgroundLockImageView = it.findViewById(R.id.lockscreen_background_image)
            mForgroundLayout = it.findViewById(R.id.lockscreen_forground_layout)
            mShimmerTextView = it.findViewById(R.id.shimmer_tv)
            mForgroundLayout?.setOnTouchListener(mViewTouchListener)

            mStatusBackgruondDummyView = it.findViewById(R.id.lockscreen_background_status_dummy)
            mStatusForgruondDummyView = it.findViewById(R.id.lockscreen_forground_status_dummy)
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                val statusBarHeight = LockscreenUtil.getInstance(mContext).statusBarHeight
                val forgroundParam = mStatusForgruondDummyView?.layoutParams
                forgroundParam?.height = statusBarHeight
                ...
                val backgroundParam = mStatusBackgruondDummyView?.layoutParams
                backgroundParam?.height = statusBarHeight
                ...
                MotionEvent.ACTION_MOVE -> { // 2
                    if (isLockOpen) {
                        ...
                        if (mForgroundLayout?.x!! >= 0) {
                            mForgroundLayout?.x = (layoutPrevX + touchMoveX).toInt().toFloat()
                            mBackgroundLockImageView?.x = (layoutInPrevX + touchMoveX / 1.8).toInt().toFloat()
